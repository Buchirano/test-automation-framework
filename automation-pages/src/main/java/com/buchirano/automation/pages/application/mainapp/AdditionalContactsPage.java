...
public class AdditionalContactsPage extends LightningBasePageClass {
    ...
    /** XPath for all Relationship to Beneficiary picklist option items. */
    public final String relationshipToBeneficiaryPicklistValues =
    ...
    /** XPath for the Tax ID required field error message. */
    public final String taxIdErrorMessageText = "//lightning-input/div[2]";
    ...
    /** XPath for the Relationship to Beneficiary dropdown button. */
    public final String relationshipToBeneficiaryDropDown =
            "//*[@data-id='rowRelationshipToVeteran']//div/button";

    /** XPath for the Is Primary Beneficiary checkbox. */
    public final String isPrimaryBeneficiaryCheckbox = getInputXPathByDataId("rowIsVeteran");
    ...
    /** XPath for the identifier input field. */
    public final String taxId = getInputXPathByDataId("visibleSSN");
    ...
    /** @return list of Relationship to Beneficiary dropdown buttons across all contact rows */
    public List<AutomatedObject> getRelationshipToBeneficiaryDropDown() {
        return getElementsByXPath(relationshipToBeneficiaryDropDown);
    }

    /** @return list of Is Primary Beneficiary checkboxes across all contact rows */
    public List<AutomatedObject> getIsPrimaryBeneficiaryCheckbox() {
        return getElementsByXPath(isPrimaryBeneficiaryCheckbox);
    }
    ...
    /** @return list of identifier input fields across all contact rows */
    public List<AutomatedObject> getTaxId() { return getElementsByXPath(taxId); }
    ...
    /** @return list of all Relationship to Beneficiary dropdown option items */
    public List<AutomatedObject> getRelationshipToBeneficiaryDropdownOptions() {
        return getElementsByXPath(relationshipToBeneficiaryPicklistValues);
    }
    ...
    public void enterTaxId(String taxId) { getTaxId().get(indexToUse).sendKeys(taxId); }

    public void clearTaxId() { getTaxId().get(indexToUse).clear(); }
    ...
    public void enterNewContactAdditionalInfo(String birthDate, String relationship,
            boolean isPrimaryBeneficiary, boolean setAside) {
        setIndexToUse();
        fillInputField(getBirthday().get(indexToUse), birthDate);
        if (relationship != null) {
            getRelationshipToBeneficiaryDropDown().get(indexToUse).click();
            WebElement element = driver.findElement(
                    By.xpath("//span[@title = '" + relationship + "']"));
            scrollIntoView(element);
            element.click();
        }
        if (isPrimaryBeneficiary) { getIsPrimaryBeneficiaryCheckbox().get(indexToUse).click(); waitForSalesforceLoad(); }
     ...
     * @param relationshipToBeneficiary {@link Relationship} enum value; skipped if null
     */
    public void enterNewContact(String firstName, String middleName, String lastName,
            String taxId, String birthDate, Suffix suffix, Relationship relationshipToBeneficiary) {
        setIndexToUse();
        ...
        if (!StringUtils.isBlank(taxId)) fillInputField(getTaxId().get(indexToUse), taxId);
        ...
        if (relationshipToBeneficiary != null) selectDropdownOption(
                getRelationshipToBeneficiaryDropDown().get(indexToUse), relationshipToBeneficiary.getText());
    }
    ...
    /** @return the identifier value from the first contact row */
    public String retrieveTaxId() { return getTaxId().get(0).getPropertyValue("value"); }

    /** @return the Relationship to Beneficiary value from the first contact row */
    public String retrieveRelationshipToBeneficiary() {
        return getRelationshipToBeneficiaryDropDown().get(0).getPropertyValue("value");
    }
    ...
    /** Clicks the Relationship to Beneficiary dropdown on the current new contact row. */
    public void clickRelationshipToBeneficiaryField() {
        setIndexToUse();
        getRelationshipToBeneficiaryDropDown().get(indexToUse).click();
    }
    ...
    public List<AutomatedObject> listOfAllFields(int contactRow) {
        ...
        return List.of(
                ...
                getRelationshipToBeneficiaryDropDown().get(idx)
        );
    }
}
